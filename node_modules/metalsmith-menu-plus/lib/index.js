...
function searchBreadcrumbs(urlPath, navigation, currentPath) {
  ...
  for (const item of navigation) {
    ...
    if (item.children && item.children.length && (urlPath.startsWith(`${normalizePath(item.path)}/`) || item.path === '/' && urlPath !== '/')) {
      // Add this item to the current path and search its children
      // Skip adding root again if it's already in the path
      const newPath = item.path === '/' && currentPath.some(p => p.path === '/') ? currentPath : [...currentPath, {
        title: item.title,
        path: item.path
      }];
...
function createNavigationStructure(paths, files, options) {
  ...
  function processDirectory(dirName, dirData, dirPath) {
    ...
    (_dirData$__files = dirData.__files) == null || _dirData$__files.forEach(path => {
  ...
  function processTree(tree) {
    var _tree$__files, _tree$__files3;
    const items = [];

    // Add the home/index item if it exists
    const rootIndex = (_tree$__files = tree.__files) == null ? void 0 : _tree$__files.find(f => f === 'index.html');
    if (rootIndex) {
    ...
    Object.keys(tree).forEach(dirName => {
      if (dirName !== '__files' && dirName !== '__dirs') {
        ...
        const hasMatchingFile = (_tree$__files2 = tree.__files) == null ? void 0 : _tree$__files2.includes(dirFile);
    ...
    (_tree$__files3 = tree.__files) == null || _tree$__files3.forEach(path => {
...
export { navigationPlugin as default };
