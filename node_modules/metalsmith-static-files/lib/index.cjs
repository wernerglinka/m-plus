...
function globToRegex(pattern) {
  ...
  const escaped = pattern.replace(/[.+^${}()|[\]\\]/g, '\\$&').replace(/\*/g, '[^/]*') // * should not match path separators
...
async function collectFiles(dir, baseDir, ignorePatterns, debug) {
  ...
  for (const entry of entries) {
    const absolutePath = path__default["default"].join(dir, entry.name);
    const relativePath = path__default["default"].relative(baseDir, absolutePath);
  ...
  return function metalsmithStaticFiles(files, metalsmith, done) {
    try {
      ...
      if (!deprecationWarningShown) {
        ...
        console.warn('\x1b[33m%s\x1b[0m', '[DEPRECATED] metalsmith-static-files is deprecated. ' + 'Use Metalsmith 2.7\'s built-in statik() method instead:\n' + '  metalsmith.statik("assets/**")\n' + 'See: https://github.com/metalsmith/metalsmith/issues/361');
      }
      ...
      collectFiles(source, source, options.ignore, debug).then(async fileList => {
        for (const {
          relativePath,
          absolutePath
        } of fileList) {
          // Build the destination key (path in the files object)
          const destKey = path__default["default"].join(options.destination, relativePath);
          ...
          const contents = await fs__default["default"].readFile(absolutePath);
          const stats = await fs__default["default"].stat(absolutePath);
...
module.exports = metalsmithStaticFiles;
