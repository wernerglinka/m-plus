...
var route__namespace = /*#__PURE__*/_interopNamespace(route);
...
function dateFormatter(format, locale) {
...
const dupeHandlers = {
  error(targetPath, filesObj, filename, opts) {
    const target = path__default["default"].join(targetPath, opts.directoryIndex);
    const currentBaseName = path__default["default"].basename(filename);
  ...
  index(targetPath, filesObj, filename, opts) {
    let target,
      ...
      postfix = '';
    do {
      target = path__default["default"].join(`${targetPath}${postfix}`, opts.directoryIndex);
      postfix = `-${++counter}`;
    } while (filesObj[target]);
    return target;
  },
  overwrite(targetPath, filesObj, filename, opts) {
    return path__default["default"].join(targetPath || '', opts.directoryIndex);
  }
};
...
function slugFn(options = defaultLinkset.slug) {
  ...
  if (typeof options.extend === 'object' && options.extend !== null) {
    slugify__default["default"].extend(options.extend);
  }
  return function defaultSlugFn(text) {
    return slugify__default["default"](text, options);
  };
}
const normalizeLinkset = (linkset, defaultLs = defaultLinkset) => {
  ...
  if (typeof linkset.date !== 'function') {
    linkset.date = typeof linkset.date === 'string' ? dateFormatter(linkset.date, defaultLs.date.locale) : dateFormatter((linkset || defaultLs).date.format, linkset.date.locale);
  }
  return linkset;
};
...
const replace = ({
  ...
  const {
    keys
  } = route__namespace.parse(remapped);
  const ret = {};
  for (let i = 0, key; key = keys[i++];) {
    const keypath = key.replace(/\0/g, '.');
    const val = get__default["default"](data, keypath);
  ...
  let transformed = route__namespace.inject(remapped, ret);
  if (path__default["default"].basename(transformed) === path__default["default"].basename(options.directoryIndex, path__default["default"].extname(options.directoryIndex))) transformed = path__default["default"].dirname(transformed);
...
function permalinks(options) {
  ...
  return function permalinks(files, metalsmith, done) {
    ...
    metalsmith.match(patternMatch, Object.keys(files)).forEach(file => {
      ...
      const permalinkTransformContext = {
        ...normalizedOptions,
        ...defaultLinkset,
        ...linkset
      };
      ...
      try {
        ppath = replace(permalinkTransformContext, {
          ...data,
          basename: path__default["default"].basename(file, path__default["default"].extname(file)),
          dirname: path__default["default"].dirname(file) === '.' ? '' : path__default["default"].dirname(file)
        });
      } catch (err) {
      ...
      const out = makeUnique(path__default["default"].normalize(ppath), files, file, normalizedOptions);
